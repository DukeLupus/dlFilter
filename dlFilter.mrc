...
on me:*:join:#: {
  ...
  .timer 1 1 DLF.@find.ColourMeTimer $event $chan
  if ($DLF.Chan.IsDlfChan($chan)) {
...
; Requeue timer if inwho
alias -l DLF.@find.ColourMeTimer {
  if ($chan($2).inwho) .timer 1 1 DLF.@find.ColourMeTimer $1-2
  else DLF.@find.ColourMe $1-2
}
